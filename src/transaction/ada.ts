...
import {
  AdaBlockChainInfo,
  AdaSendTransactionCallback, AdaTransaction,
  AdaUtxo,
  Currency,
  FromAddress,
  FromUTXO,
...
const processFeeAndRest = async ({ txBuilder, fromAmount, toAmount, transferBtcBasedBlockchain, adaGetTransaction }: ProcessFeeAndRest) => {
  const { fromAddress, fromUTXO } = transferBtcBasedBlockchain
  if (fromAddress) {
...
const signTransaction = ({ txBuilder, privateKeysToSign, transferBtcBasedBlockchain }: SignTransaction) => {
